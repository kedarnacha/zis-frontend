  ...
  return (
    <DataLayout isError={isError} isLoading={isLoading || isFetching} className="h-[30dvh]">
      ...
      {(data?.data && data.data.length > 0) ? (
          data.data.map((item) => (
            <PengajuanItemCardVertical key={item.program_id} program={item} />
          ))
        ) : (
...
export default MustahiqProgramList;
